...
function getGraphqlTableDefinition(tsTableDefAttr, tableName) {
  var tableDef = '\n# table: ' + tableName + '\n';
  tableDef += tsTableDefAttr + '\n}\n';
  tableDef += '\ntype '+ _.upperFirst(_.camelCase(tableName)) +'Result {\n'
  tableDef += '  count: Int!\n'
  tableDef += '  rows: ['+_.upperFirst(_.camelCase(tableName)) + ']\n}\n'
  return tableDef;
}
...
function getMemberGraphqlSchemaDef(spaces, fieldName, val, allowNull) {
  if (fieldName === undefined) return '';
  var m = '\n' + spaces + fieldName + ":";
  ...
  if(fieldName == "id"){
    ...
    m += 'String' + (allowNull === true ? '' : '!');
  } else if (val.indexOf('DataTypes.BOOLEAN') > -1) {
    m += 'Bollean' + (allowNull === true ? '' : '!');
  } else if (val.indexOf('DataTypes.INTEGER') > -1) {
    m += 'Int' + (allowNull === true ? '' : '!');
  } else if (val.indexOf('DataTypes.BIGINT') > -1) {
    m += 'Int' + (allowNull === true ? '' : '!');
  } else if (val.indexOf('DataTypes.STRING') > -1) {
    m += 'String' + (allowNull === true ? '' : '!');
...
exports.def = {
  ...
  getTableDefinition,
  getMemberDefinition,
  getMemberGraphqlSchemaDef,
  getGraphqlTableDefinition
};