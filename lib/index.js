...
console.log('init----',typescriptFiles)
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      var tsGraphqlTableDef = 'type ' + _.upperFirst(_.camelCase(table)) + ' {';
      ...
      _.each(fields, function(field, i){
        ...
        if(field == 'gmt_create'){
          text[table] += spaces + spaces + "'" + 'createdAt' + "': {\n";
        }else if(field == 'gmt_modified'){
          text[table] += spaces + spaces + "'" + 'updatedAt' + "': {\n";
        }else{
        ...
        if(self.options.typescript) tsGraphqlTableDef += tsHelper.def.getMemberGraphqlSchemaDef(spaces, fieldName, tsVal, tsAllowNull);
      });
      ...
      if(self.options.typescript) typescriptFiles[2] += tsHelper.def.getGraphqlTableDefinition(tsGraphqlTableDef, tableName);
...
AutoSequelize.prototype.write = function(attributes, typescriptFiles, callback) {
  ...
  if (self.options.typescript) {
    if (typescriptFiles !== null && typescriptFiles.length > 1) {
      console.log('tables.graphql',typescriptFiles[2])
      ...
      fs.writeFileSync(path.join(self.options.directory, 'tables.graphql'), typescriptFiles[2], 'utf8');
    }
  }
